%% extract_strains_45deg.m
%  Multi-Zone Strain Extraction and R-Value Analysis — 45° Direction
%  Specimens: 45-01, 45-02, 45-03
%
...
%
%  For 45° specimens, the shear strain Exy provides additional
%  information about off-axis coupling and is used to validate
%  the anisotropic yield criterion (Hill'48 parameter N).
%
...
%  Output: CSV strain histories, r-value summary, Exy/Eyy analysis, plots
...
for s = 1:length(specimens)
    ...
    fprintf('\n========================================\n');
    fprintf('  Processing: %s (45° direction)\n', specimen_name);
    fprintf('========================================\n');
    ...
    % --- Exy/Eyy ratio analysis (specific to 45° specimens) ---
    fprintf('\n  Shear coupling analysis (Exy/Eyy ratio):\n');
    fprintf('  %-8s  %-12s  %-12s\n', 'Zone', 'Exy/Eyy(%)', 'Assessment');
    ...
    for z = 1:Nzones
        ...
        ratio = mean(abs(results(z).Exy(valid)) ./ results(z).Eyy(valid)) * 100;
        
        if ratio < 2
            assessment = 'Weak anisotropy';
        elseif ratio < 5
            assessment = 'Moderate anisotropy';
        else
            assessment = 'Strong anisotropy';
        end
        
        fprintf('  Zone %-2d   %-12.2f  %-12s\n', z, ratio, assessment);
        specimen_exy = [specimen_exy; z, ratio];
    end
    ...
    if ~isempty(specimen_exy)
        exy_mean = mean(specimen_exy(:, 2));
        exy_summary = [exy_summary; s, exy_mean];
        fprintf('  >>> %s: mean |Exy/Eyy| = %.2f%%\n', specimen_name, exy_mean);
    end
    
    % --- Plot per specimen ---
    figure('Name', sprintf('%s — 45° Strain Fields', specimen_name), ...
           'Position', [50+200*(s-1), 100, 550, 800]);
    ...
    title('Exy (shear) — off-axis coupling');
    legend('Location', 'best'); grid on;
    yline(0, '--k');
    
    % Exy/Eyy ratio vs Eyy
    subplot(4,1,4); hold on;
    for z = 1:Nzones
        if isempty(results(z).Exy), continue; end
        valid = results(z).Eyy > 0.005;
        ratio = results(z).Exy(valid) ./ results(z).Eyy(valid) * 100;
        plot(results(z).Eyy(valid), ratio, 'DisplayName', sprintf('Z%d', z));
    end
    xlabel('\epsilon_{yy}'); ylabel('\epsilon_{xy} / \epsilon_{yy} (%)');
    title('Shear Coupling Ratio');
    legend('Location', 'best'); grid on;
    yline(0, '--k');
    
    sgtitle(sprintf('%s — 45° Direction Multi-Zone Analysis', specimen_name));
...
fprintf('\n\n');
fprintf('============================================\n');
fprintf('  45° DIRECTION — R-VALUE SUMMARY (r₄₅)\n');
fprintf('============================================\n');
...
r45_weighted = sum(r_summary(:, 2) .* weights) / sum(weights);
...
r45_robust = sum(r_vals(inlier) .* weights(inlier)) / sum(weights(inlier));
r45_std_robust = std(r_vals(inlier));
...
if n_outliers > 0
    fprintf('  Robust (IQR):   r₄₅ = %.3f ± %.3f  (%d outlier removed)\n', ...
        r45_robust, r45_std_robust, n_outliers);
else
    fprintf('  Robust (IQR):   r₄₅ = %.3f ± %.3f  (no outliers)\n', ...
        r45_robust, r45_std_robust);
end
fprintf('============================================\n');

if ~isempty(exy_summary)
    fprintf('\n  Shear coupling: mean |Exy/Eyy| = %.2f%%\n', mean(exy_summary(:,2)));
    fprintf('  Interpretation: ');
    if mean(exy_summary(:,2)) < 2
        fprintf('Weak anisotropy — material is nearly isotropic at 45°\n');
    elseif mean(exy_summary(:,2)) < 5
        fprintf('Moderate anisotropy — measurable shear coupling at 45°\n');
    else
        fprintf('Strong anisotropy — significant off-axis shear effect\n');
    end
end

% Save for external use (e.g., compute_anisotropy.m)
r45_final = r45_robust;
save(fullfile(pwd, 'r45_result.mat'), 'r45_final', 'r_summary', 'r45_weighted', 'r45_robust');
fprintf('  Result saved to r45_result.mat\n');
